...
from .selectable import _interpret_as_from, _interpret_as_select, HasPrefixes, CTE
from .. import util
from .. import exc
...
class CTEWithDML(CTE):
    ...
    def __init__(self, element, name, _cte_alias=None, _restates=frozenset()):
        if not name:
            raise ValueError("name for CTEWithDML must be provided")
        if not isinstance(element, UpdateBase):
            raise ValueError("Element for CTEWithDML must be an INSERT, UPDATE, or DELETE statement")
        if not element._returning:
            raise ValueError("DML statement for CTEWithDML must have explicit returning() columns")
        self._cte_alias = _cte_alias
        self._restates = _restates
        super(CTEWithDML, self).__init__(element, name)
