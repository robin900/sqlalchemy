...
class OnConflictTest(fixtures.TestBase, AssertsExecutionResults, AssertsCompiledSQL):
    ...
    def test_on_conflict_do_update(self):
        ...
        users.create()
        try:
            users.insert(postgresql_on_conflict=DoUpdate().on_columns(users.c.id).set_with_excluded(users.c.name)).execute(id=1, name='name1')
            eq_(users.select().where(users.c.id == 1)
                .execute().fetchall(), [(1, 'name1')])
            users.insert(postgresql_on_conflict=DoUpdate().on_columns(users.c.id).set_with_excluded(users.c.id, users.c.name)).execute(id=1, name='name2')
            eq_(users.select().where(users.c.id == 1)
    ...
    def test_on_conflict_do_update_exotic_targets(self):
        ...
        try:
            ...
            poc = DoUpdate().on_columns(*users.primary_key.columns).set_with_excluded(users.c.name, users.c.login_email)
            ...
            poc = DoUpdate().on_constraint(unique_constraint).set_with_excluded(users.c.id, users.c.name, users.c.login_email)
            ...
            # try unique constraint by name: cause an upsert on target login_email, not id
            poc = DoUpdate().on_constraint(unique_constraint.name).set_with_excluded(users.c.id, users.c.name, users.c.login_email)
            # note: lets_index_this value totally ignored in SET clause.
            users.insert(postgresql_on_conflict=poc).execute(id=43, name='nameunique2', login_email='name2@gmail.com', lets_index_this='unique')
            eq_(users.select().where(users.c.login_email == 'name2@gmail.com')
                .execute().fetchall(), [(43, 'nameunique2', 'name2@gmail.com', 'not')])

            # try bogus index
            try:
                users.insert(
                    postgresql_on_conflict=DoUpdate()
                        .on_columns(*bogus_index.columns, where=bogus_index.dialect_options['postgresql']['where'])
                        .set_with_excluded(users.c.name, users.c.login_email)
